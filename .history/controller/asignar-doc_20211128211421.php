...
if (isset($_POST['asignar_d'])) {
    ...
    for ($i = 0; $i < count($asesor); $i++) {
        $conexion->query("UPDATE propuesta SET tutor ='$asesor' WHERE id= '$id_p'");
    }
    ?>
    <p style="position: absolute; padding: 10px;border-radius: 10px; top: 20%; left: 650px;opacity: 1;
    text-align: center; width: 20%; background: #abff96; color: #1e9700; border: 1px #1e9700 solid;" 
    id="success">Propuesta enviada con éxito</p>
    <script>
        setTimeout(function() {
