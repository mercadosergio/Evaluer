...
if (isset($_POST['enviar'])) {
	...
	$ruta = "pages/files/anteproyectos/" . $nombre_final;
	...
	if (!file_exists('../files/anteproyectos')) {
		...
		if (move_uploaded_file($guardado, '../files/anteproyectos/' . $nombre)) {
			...
			$conexion->query("INSERT INTO anteproyecto(nombre,documento,comentarios,remitente) VALUES('$nombre','$ruta','$comentario','" . $_SESSION['usuario'] . "')");


			$conexion->query("UPDATE anteproyecto a
            JOIN estudiante es ON a.remitente = es." . $_SESSION['usuario'] . " 
            SET a.programa_id = es.programa_id");

			$conexion->query("UPDATE anteproyecto a
            JOIN estudiante es ON a.programa_id = es." . $_SESSION['usuario'] . " 
