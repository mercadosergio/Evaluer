...
?>
        ...
        if ($tupla['id_rol'] == 1) {
            ...
            include_once("../pages/main-coordinador.php");
        } else if ($tupla['id_rol'] == 3) {
            header("location: ../pages/main-estudiante.php");
        } else if ($tupla['id_rol'] == 4) {
            include_once("../pages/main-docente.php");
        }
        mysqli_close($conexion);
    } else {
        ?>
        <p style="position: absolute; padding: 10px;border-radius: 10px; top: 20%; left: 650px;opacity: 1;
		text-align: center; width: 14%; background: rgb(255, 133, 133); color: rgb(184, 0, 0); border: 1px #1e9700 solid;" id="fail">La contraseña nueva contraseña es la misma que la anterior</p>
        <script>
            setTimeout(function() {
                $('#fail').fadeOut('fast');
            }, 2000); // <-- time in milliseconds
...
<?php
        header("location: ../support/change-password");
    }
}
