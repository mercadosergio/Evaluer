    ...
    for ($j = 0; $j < count($num_integrantes); $j++) {
        for ($i = 0; $i < count($programa_id); $i++) {
            ...
            if ($num_integrantes[$j] == 1) {
                $objStudent->EnviarPropuesta($titulo, $linea, intval($num_integrantes[$j]), $tutor, $lider, $programa_id[$i], $semestre, $descripcion, $miembro1, '', '', $fecha, $$_SESSION['usuario']);
            } else if ($num_integrantes[$j] == 2) {
                $miembro2 = $_POST['miembro2'];
                $objStudent->EnviarPropuesta($titulo, $linea, intval($num_integrantes[$j]), $tutor, $lider, $programa_id[$i], $semestre, $descripcion, $miembro1, $miembro2, '', $fecha, $$_SESSION['usuario']);
            } else {
                $miembro2 = $_POST['miembro2'];
                $miembro3 = $_POST['miembro3'];
                $objStudent->EnviarPropuesta($titulo, $linea, intval($num_integrantes[$j]), $tutor, $lider, $programa_id[$i], $semestre, $descripcion, $miembro1, $miembro2, $miembro3, $fecha, $$_SESSION['usuario']);
            }
        }
    }
